...
class Base {
  ...
  initPlayList() {
    ...
    .set('r8', {
      bonus: 9,
      tip: '从01~11中任选8个或多个号码，选号与奖号五个相同，即中奖<em class="red">9</em>元',
      name: '任八'
    })
  }
  ...
  getCount() {
    ...
    if (count === 0) {
      ...
      tpl = `您选了<b>${count}</b>注，共<b>${count * 2}</b>元 <em>若中奖，奖金：<strong class="red">${range[0]}</strong>元，您将${win1 >= 0 ? '盈利' : '亏损'} <strong class="">166</strong>元</em>`
    }
    else {
      tpl = `您选了<b>${count}</b>注，共<b>${count * 2}</b>元<em>若中奖，奖金：<strong class="red">${range[0]}</strong>至<strong class="red">${range[1]}</strong>至<strong class="${win2 >= 0 ? 'red' : 'green'}">${c1}</strong>元</em>`
    }
    $('.sel_info').html(tpl);
  }
